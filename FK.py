...
def c_FK_unit(origin,Ratio_ca=1.9,PBC=False):
...
def c_FK(nx,ny,Ratio_ca=1.9):
    ...
    for i in range(nx):
        for j in range(ny):
            ...
            c_unit_cells.append(c_FK_unit(origin_ij,Ratio_ca,PBC=True))
...
def c_FK_unit_B(origin,Ratio_ca=1.9,PBC=False):
...
def c_FK_B(nx,ny,Ratio_ca=1.9):
    ...
    for i in range(nx):
        for j in range(ny):
            origin_ij = shift_x*i+shift_y*j
            c_unit_cells.append(c_FK_unit_B(origin_ij,Ratio_ca,PBC=True))
    c_wht, c_blu, c_ylw = stack_coords(c_unit_cells)
