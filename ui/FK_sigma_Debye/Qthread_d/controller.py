...
class MainWindow_controller(QtWidgets.QMainWindow):
    ...
    def setup_control(self):
        ...
        self.ui.checkBox_disp_410.stateChanged.connect(self.check_410_changed)
        self.ui.checkBox_disp_002.stateChanged.connect(self.check_002_changed)
        self.ui.pushButton_save_plot.clicked.connect(self.save_plot)
    ...
    def buttonClicked(self):
        ...
        self.d_c = float(self.ui.lineEdit_d_c.text())
    ...
    def check_410_changed(self):
        if self.done:
            print('replot')
            self.plot_results()

    def check_002_changed(self):
        if self.done:
            print('replot')
            self.plot_results()
    ...
    def coordinates_inputs(self):
        ...
        self.d_001 = 2*np.pi/np.linalg.norm(np.array([0,0,1])@d_s)

    def eval_Debye_SQ(self):
        '''
        Create an additional thread (qthread) to evaluate S(Q) and update progress
        main_thread ----------------------------- scattering_finished() 
                       |                                 ^
                       L---- qthread.start() ---- qthread.finished() 
        '''
    ...
    def plot_results(self):
        ...
        self.ui.widget.canvas behaved like the normal matplotlib figure
        """
        ## plot results
        ...
        if not hold:
            ...
            ax.cla()
        # plot S(Q) curve
        d_scale = self.d_410/self.d_c
        ax.plot(self.qq*d_scale,self.S_q,'-',color='r')
        # plot (hkl) scattering peak
        self.check_410_status = self.ui.checkBox_disp_410.isChecked()
        self.check_002_status = self.ui.checkBox_disp_002.isChecked()
        if self.check_410_status:
            ax.plot([self.Q_410*d_scale,self.Q_410*d_scale],[-40,40],'-',color='b')
        if self.check_002_status:
            ax.plot([self.Q_002*d_scale,self.Q_002*d_scale],[-40,40],'-',color='g')
        # figure settings
        index_qq = (self.qq*self.d_410>4)*(self.qq*self.d_410<8)
        max_S_q = np.max(self.S_q[index_qq])
        ax.set_xlim(2/self.d_c,15/self.d_c)
    ...
    def save_plot(self):
        if self.done:
            fname = self.filepath+'SQ_{}_{}.png'.format(self.r_ca,self.d_c)
            ax = self.ui.widget.canvas.axis
    ...
    def save_results(self):
        ## postprocessing
        # unscaled
        Q_abs = self.qq*self.d_001/self.d_c
        # Interpolation        
        self.output_SQ_unscaled = np.vstack([Q_abs,self.S_q]).T
        np.savetxt(self.filepath+'SQ_{}_{}.csv'.format(self.r_ca,self.d_c), self.output_SQ_unscaled, delimiter=',')
        ...
        check_scaled = self.ui.checkBox_scaled.isChecked()
        if check_scaled:
